...
async def list_chats(db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(ChatThread).order_by(ChatThread.updated_at.desc(), ChatThread.id.desc())
    )
...
async def create_chat(
    payload: ChatThreadCreateSchema | None = None, db: AsyncSession = Depends(get_db)
):
    chat = ChatThread(
...
async def send_message_stream(
    msg: ChatMessageSchema, db: AsyncSession = Depends(get_db)
):
    ...
    async def event_generator():
        ...
        while True:
            ...
            try:
                ...
                event_count += 1
                logger.debug(
                    f"Event {event_count} for job {job_id}: {event.get('type')}"
                )
...
async def pull_model(payload: ModelPullRequestSchema):
    """Pull a model from Ollama; defaults to the currently selected model."""
    requested_model = payload.model or ollama_service.model
    data = worker_service.start_model_pull(requested_model)
    return {
        "ok": True,
...
async def pull_model_status():
    ...
    return worker_service.get_model_pull_status()
